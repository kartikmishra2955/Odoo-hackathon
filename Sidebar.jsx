const NAV_ITEMS = [
  ...
  {
    section: "System",
    items: [{ key: "organization", label: "Organization" }, { key: "analytics", label: "Analytics" }],
  },
];
    ...
    <aside className="sticky top-0 flex h-screen w-[236px] flex-shrink-0 flex-col border-r border-border-soft bg-panel p-4 pt-[22px]">
      ...
      {NAV_ITEMS.map((group) => (
        <div key={group.section}>
          <div className="px-2.5 pb-2 pt-4 text-[10.5px] font-semibold uppercase tracking-wider text-ink-faint">{group.section}</div>
          {group.items.map((item) => {
            ...
            return (
              <button
                ...
                className={`relative flex w-full cursor-pointer items-center gap-2.5 rounded-lg px-2.5 py-2.5 text-left text-[13.5px] font-medium transition-colors ${
                  isActive ? "bg-accent-soft text-[#CBDBFF] before:absolute before:-left-4 before:top-2 before:bottom-2 before:w-[3px] before:rounded-r before:bg-accent" : "text-ink-dim hover:bg-panel-2 hover:text-ink"
                }`}
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="flex-shrink-0 opacity-85">{item.icon || <NavIcon />}</svg>
                {item.label}
              </button>
            );
          })}
      ...
      <div className="mt-auto flex items-center gap-2.5 rounded-[10px] border border-border-soft bg-panel-2 p-2.5">
        <div className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-accent to-[#8B5CF6] text-[11.5px] font-bold text-white">{initials}</div>
        <div>
          <div className="text-[12.5px] font-semibold">{user?.name || "Guest"}</div>
          <div className="text-[10.5px] text-ink-faint">{user?.role || "Employee"}</div>
        </div>
      </div>
    </aside>
  );
